...
func newKnotLocation(head: Point, tail: Point) -> Point {
    switch (head.x - tail.x, head.y - tail.y) {
        ...
        case (+1 ... +2, +2): return Point(x: tail.x + 1, y: tail.y + 1)
        case (+2, +1 ... +2): return Point(x: tail.x + 1, y: tail.y + 1)
        case (+1 ... +2, -2): return Point(x: tail.x + 1, y: tail.y - 1)
        case (+2, -2 ... -1): return Point(x: tail.x + 1, y: tail.y - 1)
        case (-2 ... -1, +2): return Point(x: tail.x - 1, y: tail.y + 1)
        case (-2, +1 ... +2): return Point(x: tail.x - 1, y: tail.y + 1)
        case (-2 ... -1, -2): return Point(x: tail.x - 1, y: tail.y - 1)
        case (-2, -2 ... -1): return Point(x: tail.x - 1, y: tail.y - 1)
...
input.split(separator: "\n").flatMap { 
    ...
    (1...9).forEach { 
        knots[$0] = newKnotLocation(head: knots[$0 - 1], tail: knots[$0])
    }
}
