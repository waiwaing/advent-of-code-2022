...
class Monkey {
    ...
    init(items: String, operation: String, divisor: String, routing: String) {
        ...
        self.operation = { (x: Int, lcm: Int) -> Int in
            (NSExpression(format: operation.replacingOccurrences(of: "x", with: String(x)))
                .expressionValue(with: nil, context: nil)) as! Int % lcm
        }
    }
}

func turn(monkeys: [Monkey], monkey: Monkey, lcm: Int) -> () {
    monkey.items.forEach { (worryLevel: Int) in
        let newWorryLevel: Int = monkey.operation(worryLevel, lcm)
...
func round(monkeys: [Monkey], lcm: Int) -> () {
    monkeys.forEach { turn(monkeys: monkeys, monkey: $0, lcm: lcm) }
}
...
let lcm = monkeys.map { $0.divisor }.reduce(1, *)
(0..<10000).forEach { _ in round(monkeys: monkeys, lcm: lcm) }
